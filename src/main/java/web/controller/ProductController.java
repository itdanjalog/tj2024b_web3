...
public class ProductController {
    ...
    public ResponseEntity<Boolean> registerProduct(
        ...
        int loggedInMemberMno; // 실제 mno를 저장할 변수
        ...
        try {
            ...
            loggedInMemberMno = memberService.info(token).getMno();

            // 만약 getMnoFromToken이 유효하지 않은 경우 특정 값(예: -1)이나 예외를 반환/발생 시킬 수 있음
            if (loggedInMemberMno <= 0) { // mno가 유효하지 않은 값일 경우 (서비스 로직에 따라 다름)
        ...
        boolean result = productService.registerProduct(productDto, loggedInMemberMno);

        if (result) {
            return ResponseEntity.ok(true); // 성공 시 200 OK
        } else {
    ...
    public ResponseEntity<List<ProductDto>> getAllProducts(
        ...
        return ResponseEntity.ok(productList); // 200 OK 와 제품 목록 반환
    }
    ...
    public ResponseEntity<ProductDto> getProduct(@RequestParam long pno) {
        ...
        if (productDto != null) {
            return ResponseEntity.ok(productDto); // 200 OK 와 제품 정보 반환
        } else {
    ...
    public ResponseEntity<Boolean> deleteProduct(
        ...
        int loggedInMemberMno;
        try {
            // 토큰에서 mno 추출
            loggedInMemberMno = memberService.info(token).getMno();
            if (loggedInMemberMno <= 0) {
        ...
        boolean result = productService.deleteProduct(pno, loggedInMemberMno);
    ...
    public ResponseEntity<Boolean> deleteProductImage(
        ...
        boolean result = productService.deleteProductImage(ino, loggedInMemberMno);
    ...
    @PutMapping("") // URL 예: PUT /product?pno=3
    public ResponseEntity<Boolean> updateProduct(
            @ModelAttribute ProductDto productDto, // 수정할 내용( pno, pname, pcontent, pprice) 및 추가할 파일(files)
            @RequestHeader("Authorization") String token) {
        ...
        boolean result = productService.updateProduct(productDto , productDto.getFiles(), loggedInMemberMno);
